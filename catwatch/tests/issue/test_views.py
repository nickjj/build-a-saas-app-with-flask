...
from catwatch.tests.lib.util import login
from catwatch.tests.lib.assertions import assert_status_with_message
from catwatch.blueprints.issue.models import Issue


class TestSettings:
    def test_support_page(self, session, client):
        """ Support page renders successfully. """
        response = client.get(url_for('issue.support'))

        assert response.status_code == 200

    def test_support_page_while_logged_in(self, session, client):
    ...
    def test_support(self, session, client):
        ...
        response = client.post(url_for('issue.support'), data=issue,
    ...
    def test_support_invalid_label(self, session, client):
